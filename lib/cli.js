...
function getDockerDir() {
  ...
  try {
    const dockerJson = require.resolve('flamingo-docker/docker.json')
...
if (!['init', 'up', 'down', 'logs', 'run'].includes(process.argv[2])) {
  ...
  switch (cmd) {
    ...
    case 'up': {
      ...
      if (!fs.existsSync(envJsonPath)) {
        ...
        fs.writeFileSync(envJsonPath, JSON.stringify(defaults, null, 2) + '\n')
        console.log('✅ Auto-created env.json at', envJsonPath)
      }

      console.log('=== 🦩 Flamingo Docker Up ===')
    ...
    case 'down': {
      const dockerRun = require('./docker-run')
      console.log('=== 🦩 Flamingo Docker Down ===')
    ...
    case 'run': {
      if (!arg1) {
        console.error('❌ Usage: fw run <scenario.json>')
        process.exit(1)
      }
      const scenarioPath = path.resolve(process.cwd(), arg1)
    ...
    default: {
      if (!cmd) {
        ...
        console.log('  fw up [ui-mode]     # install docker if needed + build & run container')
        console.log('  fw down             # stop & remove docker container')
        console.log('  fw logs             # tail docker container logs')
        console.log('  fw run <file.json>  # run a network scenario from a JSON file')
